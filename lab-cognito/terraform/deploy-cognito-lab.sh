...
sleep 40
SERVICE_NAME="pill-cognitoapp"
TASK_ARN=$(aws ecs list-tasks --service-name "$SERVICE_NAME" --query 'taskArns[0]' --output text --cluster 'pill-cognito-ecs-cluster' --region us-east-1)
TASK_DETAILS=$(aws ecs describe-tasks --task "${TASK_ARN}" --query 'tasks[0].attachments[0].details' --cluster 'pill-cognito-ecs-cluster' --region us-east-1)
...
|___| (____  /  \______  /     |_______ (____  /___  /
           ...
           _  _  ________
        __| || |_\_____  \\            The Danger of Trusting
        \   __   //  ____/       -    Unrestricted User Attributes
         |  ||  |/       \\            In AWS Cognito User Pools
        /_  ~~  _\_______ \\
          |_||_|         \/
_________________________________________________________________________________________
